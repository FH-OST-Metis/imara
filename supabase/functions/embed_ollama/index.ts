...
// For local development, Ollama runs in Docker at host.docker.internal:11434
const openai = new OpenAI({
  baseURL: Deno.env.get('OLLAMA_BASE_URL') || 'http://host.docker.internal:11434/v1',
  apiKey: Deno.env.get('OLLAMA_API_KEY') || 'ollama', // Required by client, but unused by Ollama
})
 ...
 * Uses BGE-M3 model which outputs 1024-dimensional embeddings (vs OpenAI's 1536).
 * BGE-M3 provides multi-lingual, multi-functional embeddings for semantic search.
 */
async function generateEmbedding(text: string) {
  const response = await openai.embeddings.create({
    model: Deno.env.get('OLLAMA_MODEL') || 'bge-m3:567m',
    input: text,
  })
