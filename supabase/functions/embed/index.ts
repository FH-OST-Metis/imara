 ...
 * Google Doc: https://ai.google.dev/gemini-api/docs/embeddings#javascript
 */
async function generateEmbedding(text: string) {
  const response = await gemini.models.embedContent({
    model: 'gemini-embedding-001',
    // contents retrieval is diffrent to OpenAI Supabase example, because:
    // Gemini is a multimodal model. So content could have multiple parts (e.g. part 1: picture, part 2: text)
    contents: [
      {
        parts: [
          { text: text}
    ...
    config: {
      // On my opinion, RETRIEVAL_DOCUMENT is the best joice. SEMANTIC_SIMILARITY if its a classifcation task.
      taskType: 'RETRIEVAL_DOCUMENT',
      // Optional: 3072 is default value, too. I put it here for clarity.
      outputDimensionality: 3072,
    }
  })
  const embedding = response.embeddings ? response.embeddings[0] : null;

  if (!embedding || !embedding.values) {
    console.error('Gemini Response Structure:', JSON.stringify(response));
    throw new Error('failed to generate embedding: format mismatch')
  }
