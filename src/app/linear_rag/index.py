...
class LinearRAGIndexer:
    ...
    def process_chunks(self) -> None:
        ...
        if not db_passages:
            logger.warning("No passages found in document_chunk table with chunk_hash_id")
            return
        ...
        passage_entities_map: Dict[str, List[str]] = {}  # passage_hash -> [entity_hashes]
        passage_sentences_map: Dict[str, List[str]] = {}  # passage_hash -> [sentence_hashes]
        sentence_entities_map: Dict[str, List[str]] = {}  # sentence_hash -> [entity_hashes]
        ...
        for row in db_passages:
            ...
            for sent, sent_hash in zip(sentences, sentence_hashes):
                ...
                sent_entities = [ent.text.strip() for ent in sent_doc.ents if ent.text.strip()]
                sent_entity_hashes = [compute_hash_id(ent, 'entity-') for ent in sent_entities]
        ...
        logger.info(f"Extracted {self.stats['entities_extracted']} entities ({self.stats['unique_entities']} unique)")
        logger.info(f"Extracted {self.stats['sentences_extracted']} sentences ({self.stats['unique_sentences']} unique)")
    ...
    def _insert_entities(self, entities: Set[str]) -> None:
        ...
        entity_data = [
            (entity, compute_hash_id(entity, 'entity-'))
    ...
    def _insert_sentences(self, sentences: Set[str]) -> None:
        ...
        sentence_data = [
            (sentence, compute_hash_id(sentence, 'sentence-'))
    ...
    def _insert_passage_entity_mappings(self, passage_entities: Dict[str, List[str]]) -> None:
    ...
    def _insert_sentence_entity_mappings(self, sentence_entities: Dict[str, List[str]]) -> None:
    ...
    def _build_graph(
        ...
        with self.conn.cursor() as cur:
            cur.execute("SELECT sentence_hash_id, sentence_text FROM lr_sentence_embedding")
            for row in cur.fetchall():
        ...
        entity_doc_freq = Counter()  # entity_hash -> document frequency
        ...
        passages_by_doc: Dict[str, List[Tuple[int, str]]] = {}  # title -> [(page_ref, hash)]
        for passage in passages_data:
            title = passage["title"]
            if title not in passages_by_doc:
                passages_by_doc[title] = []
            passages_by_doc[title].append((passage["page_ref"], passage["passage_hash"]))
